...
class VHDLParser:
    ...
    def __init__(self):
        ...
        self.type_pattern = re.compile(
            r"type\s+(\w+)\s+is\s+\([^)]*\);", re.MULTILINE | re.IGNORECASE
        )
        self.assignment_pattern = re.compile(
            r"(\w+)\s*<=\s*([^;]+);", re.MULTILINE | re.IGNORECASE
        )
            ...
            for target in assigned_targets:
                if target in port_names and target not in {s.name for s in signals}:
                    ...
                    signal = Signal(
                        ...
                        signal_type=(
                            SignalType.WIRE
                            if port.signal_type == SignalType.OUTPUT
                            else SignalType.WIRE
                        ),
                        width=port.width,
                        initial_value=None,
                    )
    ...
    def _parse_processes(self, body: str) -> List[Dict[str, Any]]:
        ...
        for match in self.process_pattern.finditer(body):
            ...
            if sensitivity_list:
                sensitivity_signals = [
                    s.strip() for s in sensitivity_list.split(",") if s.strip()
                ]
