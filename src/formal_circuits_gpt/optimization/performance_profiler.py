...
class PerformanceProfiler:
    ...
    def start_system_monitoring(self):
        ...
        if (
            self.system_monitor_thread is None
            or not self.system_monitor_thread.is_alive()
        ):
    ...
    def _system_monitor_loop(self):
        ...
        while not self.stop_monitoring.wait(self.sampling_interval):
            try:
                ...
                # Calculate rates
                disk_read_mb = (
                    (disk_io.read_bytes - last_disk_io.read_bytes) / 1024 / 1024
                )
                disk_write_mb = (
                    (disk_io.write_bytes - last_disk_io.write_bytes) / 1024 / 1024
                )
    ...
    def _check_performance_thresholds(self, metrics: PerformanceMetrics):
        ...
        if metrics.duration_ms > self.thresholds["slow_operation_ms"]:
            issues.append(
                f"Slow operation: {metrics.operation} took {metrics.duration_ms:.1f}ms"
            )

        if metrics.memory_mb > self.thresholds["high_memory_mb"]:
            issues.append(
                f"High memory usage: {metrics.operation} used {metrics.memory_mb:.1f}MB"
            )
        ...
        if (
            metrics.cache_hits is not None
            and metrics.cache_misses is not None
            and metrics.cache_hits + metrics.cache_misses > 0
        ):
            miss_rate = metrics.cache_misses / (
                metrics.cache_hits + metrics.cache_misses
            )
            if miss_rate > self.thresholds["cache_miss_rate"]:
                issues.append(
                    f"High cache miss rate: {miss_rate:.2%} for {metrics.operation}"
                )
    ...
    def _get_performance_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "performance_issues": {
                ...
                "slow_operation_rate": (
                    slow_operations / total_operations if total_operations > 0 else 0
                ),
            },
    ...
    def export_metrics(self, filepath: Path):
        ...
        export_data = {
            ...
            "operation_stats": {
                ...
                for op, metrics in self.operation_stats.items()
            },
