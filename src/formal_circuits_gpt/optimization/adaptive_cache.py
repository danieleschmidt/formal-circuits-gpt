...
class AdaptiveCacheManager:
    ...
    def get(self, key: str) -> Optional[Any]:
        ...
        with self._lock:
            ...
            if (
                ...
                and hasattr(item, "_compressed")
                and item._compressed
            ):
        ...
        with self._lock:
            ...
            if self.compression_enabled and self._compressor:
                try:
                    ...
                    if (
                        len(compressed_data) < len(pickle.dumps(value)) * 0.8
    ...
    def _should_evict(self, new_item_size: int) -> bool:
        ...
        return current_size >= self.max_size or current_memory >= self.max_memory_mb
    ...
    def _evict_items(self, required_space_bytes: int):
        ...
        for key in candidates:
            ...
            if (
                ...
                and self._memory_usage_mb + required_memory < self.max_memory_mb * 0.9
            ):
                break
    ...
    def _get_lru_candidates(self) -> List[str]:
        ...
        return sorted(self._cache.keys(), key=lambda k: self._cache[k].last_access)

    def _get_lfu_candidates(self) -> List[str]:
        """Get LFU eviction candidates."""
        return sorted(self._cache.keys(), key=lambda k: self._cache[k].access_count)
    ...
    def _get_cost_aware_candidates(self) -> List[str]:
        ...
        return sorted(self._cache.keys(), key=cost_benefit_ratio)
    ...
    def _get_adaptive_candidates(self) -> List[str]:
        ...
        def adaptive_score(key: str) -> float:
            ...
            features = {
                ...
                / max(current_time - item.creation_time, 1),
                "recency": 1 / max(current_time - item.last_access + 1, 1),
                "computation_cost": item.computation_cost_ms
                / 1000,  # Normalize to seconds
    ...
    def _update_adaptive_weights(self):
        ...
        recent_evictions = [
            e for e in self._eviction_history if current_time - e["timestamp"] < 1800
        ]
        ...
        avg_eviction_time = statistics.mean(
            [e["eviction_time_ms"] for e in recent_evictions]
        )
    ...
    def optimize(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            if (
                ...
                or self._memory_usage_mb > self.max_memory_mb * 0.8
            ):
