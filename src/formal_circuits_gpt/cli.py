...
@click.option(
    "--prover",
    default="isabelle",
    type=click.Choice(["isabelle", "coq"]),
    help="Theorem prover to use",
)
@click.option("--model", default="gpt-4-turbo", help="LLM model to use")
@click.option(
    "--temperature", default=0.1, type=float, help="LLM temperature (0.0-2.0)"
)
...
@click.option(
    ...
    "output_format",
    default="text",
    type=click.Choice(["text", "latex", "sva"]),
    help="Output format",
)
    ...
    try:
        with Progress(
            ...
            verifier = CircuitVerifier(
                prover=prover, model=model, temperature=temperature, debug_mode=debug
            )
            ...
            result = verifier.verify_file(
                hdl_file=hdl_file, properties=properties_list, timeout=timeout
            )
...
@click.option(
    "--prover",
    type=click.Choice(["isabelle", "coq", "all"]),
    default="all",
...
def check_setup(prover):
    ...
    if os.getenv("ANTHROPIC_API_KEY"):
        ...
        status_table.add_row(
            "Anthropic API", "⚠ Not configured", "Set ANTHROPIC_API_KEY"
        )
...
@click.option(
    "--prover", type=click.Choice(["isabelle", "coq", "both"]), default="both"
)
...
def serve(port, host, debug):
    ...
    try:
        ...
        console.print(
            "[red]✗ Flask not installed. Install with: pip install flask flask-cors[/red]"
        )
        sys.exit(1)
    except Exception as e:
...
@main.command()
@click.option(
    "--config", type=click.Path(exists=True), help="Experiment configuration file"
)
@click.option(
    "--circuits",
    type=click.Path(exists=True),
    multiple=True,
    help="Circuit files to test",
)
@click.option(
    "--provers", multiple=True, default=["isabelle"], help="Provers to compare"
)
@click.option(
    "--models", multiple=True, default=["gpt-4-turbo"], help="LLM models to test"
)
@click.option(
    "--repetitions",
    default=3,
...
def research(config, circuits, provers, models, repetitions, timeout, output_dir):
    ...
    try:
        ...
        if config:
            ...
            if not circuits:
                console.print(
                    "[red]✗ No circuits specified. Use --circuits or --config[/red]"
                )
                sys.exit(1)
        ...
        # Run experiment
        console.print(
            f"[blue]🔬 Starting research experiment: {exp_config.name}[/blue]"
        )
...
@click.option(
    "--cleanup", type=int, metavar="DAYS", help="Cleanup entries older than DAYS"
)
...
def _display_verification_result(result: ProofResult, hdl_file: str):
    ...
    if result.proof_code and len(result.proof_code) < 2000:
        ...
        syntax = Syntax(
            result.proof_code,
            "isabelle" if "theory" in result.proof_code else "coq",
            theme="monokai",
            line_numbers=True,
        )
