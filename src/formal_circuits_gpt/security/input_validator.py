...
class InputValidator:
    ...
    def validate_file_path(self, file_path: str) -> ValidationResult:
        ...
        try:
            if path.exists() and path.stat().st_size > self.max_file_size:
                errors.append(
                    f"File too large: {path.stat().st_size} bytes > {self.max_file_size}"
                )
        except OSError as e:
    ...
    def validate_hdl_content(self, hdl_content: str) -> ValidationResult:
        ...
        if len(hdl_content) > self.max_hdl_length:
            errors.append(
                f"HDL content too long: {len(hdl_content)} > {self.max_hdl_length}"
            )
    ...
    def validate_properties(
        self, properties: Union[List[str], str]
    ) -> ValidationResult:
        ...
        for i, prop in enumerate(properties):
            ...
            for pattern in self.SQL_INJECTION_PATTERNS:
                if re.search(pattern, prop, re.IGNORECASE):
                    errors.append(
                        f"Property {i+1} contains suspicious pattern: {pattern}"
                    )
                    break
            else:
                # Sanitize property
    ...
    def validate_prover_name(self, prover_name: str) -> ValidationResult:
        ...
        if prover_name.lower() not in valid_provers:
            errors.append(
                f"Unknown prover: {prover_name}. Valid options: {', '.join(valid_provers)}"
            )
    ...
    def validate_api_input(self, data: Dict[str, Any]) -> ValidationResult:
        ...
        for field, value in data.items():
            if field == "hdl_file":
                ...
                warnings.extend(
                    [f"hdl_content: {warning}" for warning in result.warnings]
                )
                if result.sanitized_input:
                    sanitized_data[field] = result.sanitized_input
                ...
                warnings.extend(
                    [f"properties: {warning}" for warning in result.warnings]
                )
                ...
                warnings.extend(
                    [f"temperature: {warning}" for warning in result.warnings]
                )
                if result.sanitized_input is not None:
                    sanitized_data[field] = result.sanitized_input
    ...
    def _attempt_structure_fix(self, text: str) -> str:
        ...
        if (
            "assign" in text.lower() or "always" in text.lower()
        ) and "module" not in text.lower():
    ...
    def validate_hdl_content(self, hdl_code: str) -> ValidationResult:
        ...
        try:
            ...
            if dangerous_found:
                if self.strict_mode:
                    ...
                    sanitized_code = self._sanitize_dangerous_patterns(
                        sanitized_code, dangerous_found
                    )
            ...
            if len(sanitized_code) > self.max_hdl_length:
                if self.strict_mode:
                    errors.append(
                        f"HDL code too long: {len(sanitized_code)} > {self.max_hdl_length}"
                    )
                else:
                    warnings.append(
                        f"HDL code is very long: {len(sanitized_code)} characters"
                    )
        ...
        return ValidationResult(
            is_valid, errors, warnings, sanitized_code if is_valid else None
        )
