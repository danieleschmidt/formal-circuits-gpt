...
class MetricsCollector:
    ...
    def export_prometheus(self) -> str:
        ...
        # Export histograms
        for name, stats in {name: self.get_histogram_stats(name) for name in self._histograms}.items():
...
class TimedOperation:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        if self.start_time:
            ...
            self.collector.record_histogram(f"{self.name}_duration_ms", duration * 1000, self.labels)
        ...
        if exc_type is None:
            self.collector.increment_counter(f"{self.name}_success_total", labels=self.labels)
        else:
            self.collector.increment_counter(f"{self.name}_error_total", labels=self.labels)
