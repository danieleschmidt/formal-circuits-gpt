...
class MetricsCollector:
    ...
    def record_verification_start(self, prover: str, circuit_name: str = "unknown"):
        ...
        self.increment_counter(
            "verification_started", {"prover": prover, "circuit": circuit_name}
        )

    def record_verification_complete(
        self,
        prover: str,
        status: str,
        duration: float,
        ...
        self.record_metric(
            ...
            duration,
            {"prover": prover, "status": status, "circuit": circuit_name},
        )

        self.record_metric(
            "properties_verified",
            properties_count,
            {"prover": prover, "circuit": circuit_name},
        )
    ...
    def get_metrics_summary(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                "performance": {
                    "min_duration": (
                        min(self._verification_durations)
                        if self._verification_durations
                        else 0
                    ),
                    "max_duration": (
                        max(self._verification_durations)
                        if self._verification_durations
                        else 0
                    ),
                    "p95_duration": self._calculate_percentile(
                        self._verification_durations, 95
                    ),
                    "p99_duration": self._calculate_percentile(
                        self._verification_durations, 99
                    ),
                },
            }
    ...
    def get_metrics_for_prometheus(self) -> str:
        ...
        lines.append(
            f"formal_circuits_uptime_seconds {metrics_summary['system']['uptime_seconds']}"
        )

        # Verification metrics
        lines.append(
            f"# HELP formal_circuits_verifications_total Total number of verifications"
        )
        lines.append(f"# TYPE formal_circuits_verifications_total counter")
        lines.append(
            f"formal_circuits_verifications_total {metrics_summary['verification']['total_verifications']}"
        )

        lines.append(
            f"# HELP formal_circuits_verification_success_rate Success rate percentage"
        )
        lines.append(f"# TYPE formal_circuits_verification_success_rate gauge")
        lines.append(
            f"formal_circuits_verification_success_rate {metrics_summary['verification']['success_rate_percent']}"
        )

        lines.append(
            f"# HELP formal_circuits_verification_duration_seconds Average verification duration"
        )
        lines.append(f"# TYPE formal_circuits_verification_duration_seconds gauge")
        lines.append(
            f"formal_circuits_verification_duration_seconds {metrics_summary['verification']['average_duration_seconds']}"
        )

        # By prover metrics
        for prover, count in metrics_summary["verification"]["by_prover"].items():
            lines.append(
                f'formal_circuits_verifications_by_prover{{prover="{prover}"}} {count}'
            )
