...
class IntelligentLoadBalancer:
    ...
    def select_worker(self, task: Any = None, estimated_duration_ms: float = 1000.0) -> Optional[str]:
        ...
        with self._lock:
            ...
            if self.strategy == LoadBalancingStrategy.ROUND_ROBIN:
                ...
                return self._select_resource_aware(available_workers, estimated_duration_ms)
            else:
    ...
    def _select_resource_aware(self, workers: List[str], estimated_duration_ms: float) -> str:
        ...
        for worker_id in workers:
            ...
            estimated_completion = estimated_duration_ms + (stats.queue_size * stats.average_response_time_ms)
        ...
        with self._lock:
            for worker_id, stats in self._workers.items():
                ...
                if worker_id in self._health_check_callbacks:
                    ...
                    recent_failures = stats.failed_tasks / max(1, stats.completed_tasks + stats.failed_tasks)
                    stats.is_healthy = (
                        recent_failures < 0.5 and  # Less than 50% failure rate
                        stats.average_response_time_ms < 30000 and  # Less than 30s response time
                        (current_time - stats.last_task_time) < 300  # Active within 5 minutes
                    )
    ...
    def get_load_balancer_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            avg_throughput = statistics.mean(self._throughput_history) if self._throughput_history else 0
            
            # Worker health summary
            healthy_workers = sum(1 for stats in self._workers.values() if stats.is_healthy)
            ...
            return {
                ...
                "total_active_tasks": sum(stats.active_tasks for stats in self._workers.values()),
                "total_completed_tasks": sum(stats.completed_tasks for stats in self._workers.values()),
                "total_failed_tasks": sum(stats.failed_tasks for stats in self._workers.values()),
                "workers": {
                    worker_id: {
