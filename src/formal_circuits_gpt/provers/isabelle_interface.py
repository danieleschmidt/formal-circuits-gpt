...
class IsabelleInterface(BaseProver):
    ...
    def check_installation(self) -> bool:
        ...
        try:
            stdout, stderr, returncode, _ = self._run_command(
                [self.prover_path, "version"]
            )
...
"""
    ...
    def verify_with_build_session(
        self, proof_content: str, session_name: str = "Circuit_Verification"
    ) -> ProverResult:
        ...
        with tempfile.TemporaryDirectory(dir=self.working_directory) as temp_dir:
            try:
                ...
                # Run Isabelle build
                command = [self.prover_path, "build", "-D", temp_dir, "-v"]
                ...
                return ProverResult(
                    success=False, output="", errors=[f"Session build failed: {str(e)}"]
                )
