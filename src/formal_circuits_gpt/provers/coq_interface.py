...
class CoqInterface(BaseProver):
    ...
    def verify_proof(self, proof_content: str) -> ProverResult:
        ...
        try:
            ...
            return ProverResult(
                success=False,
                output="",
                errors=[f"Coq verification failed: {str(e)}"]
            )
    ...
    def _parse_errors(self, stderr: str) -> List[str]:
        ...
        for line in lines:
            ...
            if any(pattern in line.lower() for pattern in [
                'error:', 'failed', 'exception', 'syntax error',
                'type error', 'unbound', 'not found', 'anomaly'
            ]):
                errors.append(line)
    ...
    def create_makefile(self, coq_files: List[str], output_dir: str) -> str:
        ...
        makefile_content = """# Coq Makefile generated by formal-circuits-gpt
        
COQC = coqc
COQDEP = coqdep

COQFILES = """ + " ".join(coq_files) + """
