...
class ProofGenerator:
    ...
    def generate_lemma(
        ...
        try:
            prompt = self.prompt_manager.create_lemma_prompt(
                lemma_statement=lemma_statement, context=context, prover=prover
            )
    ...
    def generate_inductive_proof(
        ...
        try:
            ...
            raise ProofGenerationError(
                f"Inductive proof generation failed: {str(e)}"
            ) from e
    ...
    def generate_proof_sketch(
        ...
        try:
            prompt = self.prompt_manager.create_sketch_prompt(
                goal=goal, context=context, prover=prover
            )
            ...
            raise ProofGenerationError(
                f"Proof sketch generation failed: {str(e)}"
            ) from e

    # Backward compatibility methods for tests
    def generate_initial_proof(
        self, circuit_ast, properties: List[str], **kwargs
    ) -> str:
    ...
    def construct_prompt(
        self, circuit_type: str, circuit_code: str, properties: List[str]
    ) -> str:
