...
class ResponseParser:
    ...
    def _init_isabelle_patterns(self) -> Dict[str, re.Pattern]:
        ...
        return {
            "theory_block": re.compile(
                r"theory\s+\w+.*?end", re.DOTALL | re.IGNORECASE
            ),
            "lemma_block": re.compile(
                r"lemma\s+\w+\s*:.*?(?=lemma|\Z)", re.DOTALL | re.IGNORECASE
            ),
            "proof_block": re.compile(r"proof.*?qed", re.DOTALL | re.IGNORECASE),
    ...
    def _init_coq_patterns(self) -> Dict[str, re.Pattern]:
        ...
        return {
            ...
            "lemma_block": re.compile(
                r"Lemma\s+\w+.*?Qed\.", re.DOTALL | re.IGNORECASE
            ),
            "theorem_block": re.compile(
                r"Theorem\s+\w+.*?Qed\.", re.DOTALL | re.IGNORECASE
            ),
            "proof_block": re.compile(r"Proof\..*?Qed\.", re.DOTALL | re.IGNORECASE),
        }

    def extract_proof_code(
        self, response_content: str, prover: str = "isabelle"
    ) -> str:
    ...
    def _extract_isabelle_code(self, content: str) -> str:
        ...
        theory_header = re.search(
            r"theory\s+\w+.*?begin", content, re.DOTALL | re.IGNORECASE
        )
        if theory_header:
    ...
    def validate_syntax(
        self, proof_code: str, prover: str = "isabelle"
    ) -> Tuple[bool, List[str]]:
    ...
    def _validate_coq_syntax(self, proof_code: str) -> List[str]:
        ...
        for line in lines:
            ...
            if any(
                line.startswith(kw)
                for kw in ["Definition", "Lemma", "Theorem", "Proof"]
            ):
    ...
    def extract_proof_statistics(
        ...
        stats = {
            ...
            "non_empty_lines": len(
                [line for line in proof_code.split("\n") if line.strip()]
            ),
            "lemmas": 0,
            "definitions": 0,
            "theorems": 0,
        ...
        if prover == "isabelle":
            ...
            stats["definitions"] = len(
                re.findall(r"definition\s+\w+", proof_code, re.IGNORECASE)
            )
            stats["theorems"] = len(
                re.findall(r"theorem\s+\w+", proof_code, re.IGNORECASE)
            )
