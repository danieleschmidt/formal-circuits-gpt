    ...
    async def generate(self, prompt: str, **kwargs) -> LLMResponse:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt == max_retries - 1:
                    raise LLMError(
                        f"Anthropic API error after {max_retries} attempts: {str(e)}",
                        "anthropic",
                        self.model,
                    ) from e
                await asyncio.sleep(retry_delay * (2**attempt))  # Exponential backoff
...
class LLMManager:
    ...
    def add_client(
        self, name: str, client: LLMClient, set_as_default: bool = False
    ) -> None:
        """Add LLM client."""
    ...
    async def generate(
        self, prompt: str, client_name: Optional[str] = None, **kwargs
    ) -> LLMResponse:
