...
class ProofRefiner:
        ...
        for attempt in range(max_attempts):
            try:
                ...
                if attempt == max_attempts - 1:
                    raise ProofRefinementError(
                        f"Refinement failed after {max_attempts} attempts: {str(e)}"
                    ) from e
                continue
    ...
    def fix_specific_error(
        self, proof: str, error_type: str, error_message: str, prover: str = "isabelle"
    ) -> str:
        ...
        try:
            ...
            if error_type == "syntax":
                ...
                # Generic error fix
                prompt = self.prompt_manager.create_refinement_prompt(
                    original_proof=proof, errors=[error_message], prover=prover
                )

            response = self.llm_manager.generate_sync(
                prompt=prompt, temperature=0.1, max_tokens=3000
            )
            ...
            raise ProofRefinementError(
                f"Failed to fix {error_type} error: {str(e)}"
            ) from e
    ...
    def complete_partial_proof(
        self, partial_proof: str, missing_parts: List[str], prover: str = "isabelle"
    ) -> str:
...
COMPLETED PROOF:"""
    ...
    def _create_syntax_fix_prompt(
        self, proof: str, error_message: str, prover: str
    ) -> str:
...
Please provide the completed proof with all necessary steps:"""
    ...
    def refine_failed_proof(
        self, failed_proof: str, error_message: str, attempt_number: int = 1
    ) -> str:
        ...
        if self.provider and hasattr(self.provider, "refine_proof"):
            ...
            refined_proof, success = self.refine_proof(
                original_proof=failed_proof, errors=[error_message], max_attempts=1
            )
