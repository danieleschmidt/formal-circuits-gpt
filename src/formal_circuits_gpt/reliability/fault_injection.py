...
class FaultInjector:
    ...
    def inject_network_delay(self, base_delay: float = 0.0) -> float:
        ...
        if self.should_inject_fault(FaultType.NETWORK_DELAY):
            ...
            self._record_fault(
                ...
                {"base_delay": base_delay, "injected_delay": additional_delay},
            )
    ...
    def inject_random_failure(self, operation: str) -> Optional[Exception]:
        ...
        if self.should_inject_fault(FaultType.RANDOM_FAILURE):
            ...
            return VerificationError(
                f"Injected random failure in {operation}", error_code="CHAOS_RANDOM"
            )
    ...
    def get_fault_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "by_type": by_type,
            "fault_rate": total
            / max(1, time.time() - self.fault_history[0]["timestamp"]),
        }
...
class ChaosScenarios:
    ...
    def network_instability():
        ...
        fault_injector.register_fault(
            FaultConfig(
                fault_type=FaultType.NETWORK_DELAY, probability=0.3, magnitude=2.0
            )
        )
        fault_injector.register_fault(
            FaultConfig(fault_type=FaultType.NETWORK_FAILURE, probability=0.1)
        )
    ...
    def llm_service_degradation():
        ...
        fault_injector.register_fault(
            FaultConfig(fault_type=FaultType.LLM_TIMEOUT, probability=0.2)
        )
        fault_injector.register_fault(
            FaultConfig(fault_type=FaultType.LLM_RATE_LIMIT, probability=0.15)
        )
        fault_injector.register_fault(
            FaultConfig(fault_type=FaultType.CORRUPT_RESPONSE, probability=0.05)
        )
    ...
    def prover_instability():
        ...
        fault_injector.register_fault(
            FaultConfig(fault_type=FaultType.PROVER_CRASH, probability=0.1)
        )
    ...
    def full_chaos():
        ...
        fault_injector.register_fault(
            FaultConfig(fault_type=FaultType.RANDOM_FAILURE, probability=0.05)
        )
