...
class DatabaseManager:
    ...
    def __init__(self, database_url: Optional[str] = None):
        ...
        self.database_url = database_url or os.getenv(
            "DATABASE_URL", "sqlite:///formal_circuits.db"
        )
    ...
    def _create_tables(self, conn: sqlite3.Connection):
        ...
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_proof_cache_hash ON proof_cache(circuit_hash)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_circuit_name ON circuit_models(name)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_verification_circuit ON verification_results(circuit_id)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_lemma_hash ON lemma_cache(lemma_hash)"
        )
