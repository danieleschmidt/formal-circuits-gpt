...
class PersistentCache:
    ...
    def get(self, key: str) -> Optional[Any]:
        ...
        with self.lock:
            ...
            if (
                ...
                and time.time() - metadata["created_at"] > metadata["ttl_seconds"]
            ):
                self.remove(key)
                return None
            ...
            try:
                ...
                self.index[key]["access_count"] = (
                    self.index[key].get("access_count", 0) + 1
                )
                self._save_index()
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self.lock:
            total_size = sum(
                metadata.get("size_bytes", 0) for metadata in self.index.values()
            )
...
class OptimizedCacheManager:
    ...
    def __init__(self, cache_dir: Optional[str] = None, memory_cache_size: int = 1000):
        ...
        self.logger.info(
            f"Optimized cache manager initialized with directory: {cache_dir}"
        )
    ...
    def get_proof_cache(
        self, hdl_code: str, prover: str, model: str, properties: List[str]
    ) -> Optional[Any]:
        ...
        if result is not None:
            ...
            self.memory_cache.put(
                key, result, ttl_seconds=self.proof_ttl, tags=["proof"]
            )
            return result
    ...
    def put_proof_cache(
        self, hdl_code: str, prover: str, model: str, properties: List[str], result: Any
    ):
        """Cache proof result."""
        key = self._generate_key(
            "proof",
    ...
    def get_parsed_ast_cache(self, hdl_code: str) -> Optional[Any]:
        ...
        key = self._generate_key(
            "ast", hdl_code=hashlib.sha256(hdl_code.encode()).hexdigest()
        )
