...
class BenchmarkCircuit:
    ...
    def __post_init__(self):
        ...
        if self.port_count == 0:
            ...
            self.port_count = self.hdl_code.count("input") + self.hdl_code.count(
                "output"
            )
...
class BenchmarkSuite:
    ...
    def run_comprehensive_evaluation(
        ...
        """
        print(
            f"Starting comprehensive evaluation with {len(algorithms)} algorithms on {len(self.circuits)} circuits"
        )
    ...
    def _run_algorithm_evaluation(self, algorithm_name: str, algorithm_func: Callable):
        ...
        if self.parallel_workers > 1:
            ...
            for circuit_name in circuit_names:
                try:
                    result = self._evaluate_single_circuit(
                        algorithm_name, algorithm_func, circuit_name
                    )
                    self.experiment_results.append(result)
                    print(f"  ✓ {circuit_name}: {result.verification_success}")
                except Exception as e:
    ...
    def _evaluate_single_circuit(
        self, algorithm_name: str, algorithm_func: Callable, circuit_name: str
    ) -> ExperimentResult:
        ...
        try:
            ...
            refinement_iterations = getattr(
                algorithm_result, "refinement_iterations", 0
            )
            convergence_achieved = getattr(
                algorithm_result, "convergence_achieved", False
            )
            ...
            return ExperimentResult(
                ...
                warnings=warnings,
                detailed_results=(
                    asdict(algorithm_result)
                    if hasattr(algorithm_result, "__dataclass_fields__")
                    else {}
                ),
            )
    ...
    def _analyze_circuit_performance(self) -> Dict[str, Any]:
        ...
        for circuit_name in self.circuits.keys():
            circuit_results = [
                r for r in self.experiment_results if r.circuit_name == circuit_name
            ]
    ...
    def _analyze_category_performance(self) -> Dict[str, Any]:
        ...
        for category, circuit_names in self.circuit_categories.items():
            category_results = [
                r for r in self.experiment_results if r.circuit_name in circuit_names
            ]
    ...
    def _analyze_scalability(self) -> Dict[str, Any]:
        ...
        for algorithm in algorithms:
            alg_results = [
                r for r in self.experiment_results if r.algorithm_name == algorithm
            ]

            # Extract complexity vs performance data
            complexities = [
                self.circuits[r.circuit_name].complexity_score for r in alg_results
            ]
            ...
            if len(complexities) > 1:
                ...
                scalability_analysis[algorithm] = {
                    ...
                    "scaling_trend": (
                        "linear"
                        if correlation > 0.7
                        else "sublinear" if correlation > 0.3 else "unknown"
                    ),
                }
    ...
    def _generate_recommendations(
        ...
        success_rates = statistical_analysis.algorithm_comparison.get(
            "verification_success", {}
        )
        if success_rates:
            best_algorithm = max(success_rates.items(), key=lambda x: x[1])
            recommendations.append(
                f"Best overall success rate: {best_algorithm[0]} ({best_algorithm[1]:.3f})"
            )

        # Speed recommendations
        times = statistical_analysis.algorithm_comparison.get(
            "verification_time_ms", {}
        )
        if times:
            fastest_algorithm = min(times.items(), key=lambda x: x[1])
            recommendations.append(
                f"Fastest algorithm: {fastest_algorithm[0]} ({fastest_algorithm[1]:.1f}ms avg)"
            )
        ...
        if significant_differences:
            recommendations.append(
                f"Statistically significant differences found in {len(significant_differences)} comparisons"
            )
    ...
    def _save_results(self, report: Dict[str, Any]):
        ...
        circuits_data = {
            name: asdict(circuit) for name, circuit in self.circuits.items()
        }
    ...
    def load_existing_results(self):
        ...
        for results_file in self.results_dir.glob("raw_results_*.json"):
            try:
                ...
                print(
                    f"Loaded {len(raw_results)} existing results from {results_file.name}"
                )
            except Exception as e:
