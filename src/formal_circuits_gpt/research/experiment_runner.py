...
class ExperimentRunner:
    ...
    def run_experiment(self, config: ExperimentConfig) -> List[ExperimentResult]:
        ...
        metadata = {
            ...
            "total_runs": len(config.circuits)
            * len(config.provers)
            * len(config.models)
            * config.repetitions,
        }
            ...
            for prover in config.provers:
                for model in config.models:
                    for rep in range(config.repetitions):
                        ...
                        print(
                            f"Run {run_count}/{metadata['total_runs']}: {circuit_name} + {prover} + {model} (rep {rep+1})"
                        )
                        ...
                        eta = (elapsed / run_count) * (
                            metadata["total_runs"] - run_count
                        )
                        print(
                            f"  → {result.status} ({result.duration_ms:.1f}ms) | ETA: {eta/60:.1f}min"
                        )
    ...
    def _save_result(self, result: ExperimentResult) -> None:
        ...
        results_file = (
            self.results_dir / f"experiment_{result.experiment_id}_results.jsonl"
        )
    ...
    def _calculate_statistics(self, results: List[ExperimentResult]) -> Dict[str, Any]:
        ...
        for prover in set(r.prover for r in results):
            ...
            prover_stats[prover] = {
                ...
                "success_rate": (
                    prover_verified / len(prover_results) if prover_results else 0
                ),
            }
        ...
        for model in set(r.model for r in results):
            ...
            model_stats[model] = {
                ...
                "success_rate": (
                    model_verified / len(model_results) if model_results else 0
                ),
            }
