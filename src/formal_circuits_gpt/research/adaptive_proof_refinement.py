...
class AdaptiveProofRefinement:
    ...
    def __init__(self, max_iterations: int = 20, convergence_threshold: float = 0.95,
                 learning_rate: float = 0.1, exploration_rate: float = 0.2):
        """
        ...
        self.error_strategy_mapping: Dict[ErrorCategory, Dict[ProofStrategy, float]] = defaultdict(
        ...
        self.strategy_transitions: Dict[Tuple[ProofStrategy, ProofStrategy], int] = defaultdict(int)
    ...
    def refine_proof_adaptive(self, initial_proof: str, validation_func: Callable[[str], ProverResult],
                             context: Optional[Dict[str, Any]] = None) -> Tuple[str, ConvergenceAnalysis]:
        """
        ...
        self.logger.info(f"Starting adaptive refinement session {refinement_session_id}")
        ...
        initial_confidence = self._compute_proof_confidence(current_proof, initial_result)
        ...
        for iteration in range(1, self.max_iterations + 1):
            ...
            refined_confidence = self._compute_proof_confidence(refined_proof, refined_result)
            ...
            self._update_error_strategy_mapping(error_categories, selected_strategy, refined_result.success)
            
            # Check convergence
            if refined_result.success and refined_confidence >= self.convergence_threshold:
            ...
            if refined_confidence > initial_confidence:
                ...
                self.logger.info(f"Proof improved, confidence: {refined_confidence:.3f}")
            else:
                self.logger.warning(f"No improvement in iteration {iteration}")
        
        # Max iterations reached without convergence
        self.logger.warning(f"Maximum iterations ({self.max_iterations}) reached without convergence")
    ...
    def _select_adaptive_strategy(self, error_categories: List[ErrorCategory], iteration: int,
                                 previous_attempts: List[ProofAttempt],
                                 context: Optional[Dict[str, Any]]) -> ProofStrategy:
        """
        ...
        for strategy in ProofStrategy:
            ...
            if perf.success_count + perf.failure_count > 0:
                ...
                score -= (perf.average_duration / 10000.0) * 0.1  # Prefer faster strategies
            ...
            total_attempts = sum(perf.success_count + perf.failure_count 
    ...
    def _apply_refinement_strategy(self, proof: str, strategy: ProofStrategy,
                                  error_categories: List[ErrorCategory], iteration: int) -> str:
        """
    ...
    def _apply_direct_proof_strategy(self, proof: str, errors: List[ErrorCategory]) -> str:
        ...
        for line in lines:
            ...
            if any(op in line for op in ['→', '∀', '∃', '∧', '∨']):
                enhanced_lines.append("  (* Intermediate step added by direct proof strategy *)")
        
        return '\n'.join(enhanced_lines)
    
    def _apply_contradiction_strategy(self, proof: str, errors: List[ErrorCategory]) -> str:
...
"""
    ...
    def _apply_structural_induction_strategy(self, proof: str, errors: List[ErrorCategory]) -> str:
...
"""
    ...
    def _apply_case_analysis_strategy(self, proof: str, errors: List[ErrorCategory]) -> str:
...
"""
    ...
    def _apply_lemma_decomposition_strategy(self, proof: str, errors: List[ErrorCategory]) -> str:
...
"""
    ...
    def _apply_rewrite_simplification_strategy(self, proof: str, errors: List[ErrorCategory]) -> str:
    ...
    def _apply_automated_tactics_strategy(self, proof: str, errors: List[ErrorCategory]) -> str:
...
"""
    ...
    def _categorize_errors(self, errors: List[str]) -> List[ErrorCategory]:
        ...
        for error in errors:
            ...
            if any(keyword in error_lower for keyword in ['syntax', 'parse', 'malformed']):
            ...
            elif any(keyword in error_lower for keyword in ['undefined', 'unknown', 'not found']):
                categories.append(ErrorCategory.UNDEFINED_SYMBOL)
            elif any(keyword in error_lower for keyword in ['incomplete', 'missing', 'unfinished']):
                categories.append(ErrorCategory.PROOF_INCOMPLETE)
            elif any(keyword in error_lower for keyword in ['logic', 'contradiction', 'invalid']):
            ...
            elif any(keyword in error_lower for keyword in ['tactic', 'failed', 'cannot apply']):
    ...
    def _compute_proof_confidence(self, proof: str, result: ProverResult) -> float:
        ...
        quality_score = sum(quality_indicators.get(word, 0) 
    ...
    def _update_strategy_performance(self, attempt: ProofAttempt):
        ...
        for error_cat in attempt.error_categories:
            perf.error_distribution[error_cat] = perf.error_distribution.get(error_cat, 0) + 1
        
        # Update average iterations (simplified)
        total_attempts = perf.success_count + perf.failure_count
        perf.average_iterations = (perf.average_iterations * (total_attempts - 1) + 
                                 attempt.iteration_number) / total_attempts
    
    def _update_error_strategy_mapping(self, error_categories: List[ErrorCategory],
                                     strategy: ProofStrategy, success: bool):
        ...
        for error_cat in error_categories:
            ...
            self.error_strategy_mapping[error_cat][strategy] = (
                (1 - self.learning_rate) * current_weight + self.learning_rate * weight
            )
    
    def _analyze_convergence(self, attempts: List[ProofAttempt], confidence_trajectory: List[float],
                           strategy_history: List[ProofStrategy],
                           convergence_iteration: Optional[int]) -> ConvergenceAnalysis:
        ...
        avg_errors_per_attempt = np.mean([len(attempt.error_categories) for attempt in attempts])
        theoretical_bound = min(self.max_iterations, int(math.ceil(avg_errors_per_attempt * 2)))
        
        # Error reduction rate
        if len(attempts) > 1:
            initial_errors = len(attempts[0].error_categories)
            final_errors = len(attempts[-1].error_categories)
            error_reduction_rate = (initial_errors - final_errors) / max(initial_errors, 1)
        else:
            error_reduction_rate = 0.0
    ...
    def get_convergence_statistics(self) -> Dict[str, Any]:
        ...
        avg_convergence_iteration = np.mean([ca.convergence_iteration for ca in converged_sessions
        ...
        return {
            ...
            "theoretical_efficiency": avg_convergence_iteration / avg_iterations if avg_iterations > 0 else 0
        }