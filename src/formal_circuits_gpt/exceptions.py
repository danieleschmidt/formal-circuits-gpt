...
class VerificationError(FormalCircuitsGPTError):
    ...
    def __init__(self, message: str, circuit_name: str = None, 
                 properties_failed: List[str] = None, **kwargs):
...
class ProofFailure(VerificationError):
    ...
    def __init__(self, message: str, failed_goal: str = None, 
                 counterexample: str = None, proof_attempt: str = None,
                 refinement_attempts: int = 0, **kwargs):
        ...
        if counterexample:
            self.suggestions.append("Analyze the counterexample to understand the failure")
        self.suggestions.append("Consider simplifying the property or breaking it into smaller parts")


class ParsingError(FormalCircuitsGPTError):
    """Enhanced HDL parsing error with detailed diagnostics."""
    
    def __init__(self, message: str, hdl_type: str = None, 
                 line_number: int = None, column: int = None,
                 source_snippet: str = None, **kwargs):
...
class TranslationError(FormalCircuitsGPTError):
    ...
    def __init__(self, message: str, target_language: str = None,
                 unsupported_constructs: List[str] = None, **kwargs):
        super().__init__(message, category=ErrorCategory.VERIFICATION, **kwargs)
        ...
        if unsupported_constructs:
            self.suggestions.append(f"Consider rewriting using supported {target_language} constructs")
...
class ProverError(FormalCircuitsGPTError):
    ...
    def __init__(self, message: str, prover_name: str = None,
                 prover_output: str = None, command: str = None, **kwargs):
        ...
        self.suggestions.extend([
            f"Ensure {prover_name} is properly installed and in PATH" if prover_name else "Check prover installation",
...
class LLMError(FormalCircuitsGPTError):
    ...
    def __init__(self, message: str, provider: str = None, model: str = None,
                 api_error_code: str = None, tokens_used: int = None, **kwargs):
...
class SecurityError(FormalCircuitsGPTError):
    ...
    def __init__(self, message: str, violation_type: str = None,
                 dangerous_patterns: List[str] = None, **kwargs):
...
class ConfigurationError(FormalCircuitsGPTError):
    ...
    def __init__(self, message: str, config_file: str = None,
                 missing_keys: List[str] = None, **kwargs):
        ...
        if missing_keys:
            self.suggestions.append(f"Add missing configuration keys: {', '.join(missing_keys)}")
        if config_file:
            self.suggestions.append(f"Check configuration file: {config_file}")
        self.suggestions.append("Run setup verification with 'formal-circuits-gpt check-setup'")
